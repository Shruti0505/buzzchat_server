...
const adminLogin = TryCatch(async (req, res) => {
    ...
    return res.status(200).cookie("admin-token", token, {...cookieOptions, 
        maxAge: 15 *  60  * 1000, 
...
const adminLogout = TryCatch(async (req, res,next) => {
    ...
    return res.status(200).cookie("admin-token", "", {...cookieOptions, 
        maxAge: 0, 
    ...
    const transformedUsers = await Promise.all (users.map(
        async ({name, username,avatar,_id})=>{
            const [groups, friends]=await Promise.all([
                Chat.countDocuments({ groupVhat: true, members: _id }),
